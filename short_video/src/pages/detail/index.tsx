...
import {View, Button, ScrollView, Image, Ad, Text} from 'remax/wechat';
// @ts-ignore
        ...
        <ScrollView className="scroll_wrap" scrollIntoView="toView_3" scrollWithAnimation scrollY style={{top:460}}>
            <View className="video_infos_wrap">
                ...
                <View className="video_user">
                    <View className="video_user_info" data-channelid="{{appId}}">
                        <Image mode="aspectFit" src={channelImgUrl} />
                            ...
                            <View className="flow-link" key={index}>
                                {
                                    (index+1)%9==0 ? (
                                        ...
                                        <View className="flow-linka">
                                            <View id="pic">
                                                <Image id="img" mode="widthFix" src={video.videoImage} />
                                                <Text className="video-time">{video.time}</Text>
                                            </View>
                                            <View className="info">
                                                <Text className="info-title">{video.title}</Text>
                                                <Text className="info-count">{video.channelName|| '汽车之家'}</Text>
                                            </View>
                                        </View>
                                    )
                                }
